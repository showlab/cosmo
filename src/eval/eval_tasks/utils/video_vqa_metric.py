...
import re
import sys
#from src.eval.eval_tasks.utils.text_simlarity import match_by_similarity
...
class VideoVQA:
    def __init__(self, annotation_file=None):
        ...
        if not annotation_file == None:
            print("loading Video VQA annotations into memory...")
            ...
            # add a key question_id for dataset
            # self.new_dataset = copy.deepcopy(self.dataset)
            ...
            for gtAnsDatum in gts[quesId]["answers"]:
                ...
                if similarity_match:
                    matchingAns = [
                        ...
                        if match_by_similarity(item["answer"], resAns) 
                    ]
                else:
...
def compute_video_vqa_accuracy(result_json_path, annotation_json_path):
    ...
    vqa = VideoVQA(annotation_json_path)
...
def postprocess_video_vqa_generation(predictions):
...
if __name__ == "__main__":
    # test
    result_json_path = "/datadrive_d/jinpeng/Code/videogpt4/msvd_qa_results_4e263dbe-872a-44aa-8832-5ced34eac841.json"
    annotation_json_path = "/home/jinpeng/blob/vigstandard_data/v-jinpewang/dataset/downstream_datasets/msvd/annotations/msvd_test_qa_encode.json"

    acc = compute_video_vqa_accuracy(result_json_path, annotation_json_path)
    print(acc)